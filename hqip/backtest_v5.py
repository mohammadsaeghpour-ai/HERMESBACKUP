#!/usr/bin/env python3
"""HQIP Backtest v5 — R:R Optimized (1:2 to 1:4)"""
import sys
sys.path.insert(0, "/data/workspace")
import ccxt, pandas as pd, numpy as np
from hqip.strategies_rr import analyze_15m, analyze_1h, analyze_4h, analyze_1d
from hqip.indicators import calculate_all_indicators

CCXT_SYMBOL = "BTC/USDT"
CAPITAL = 10.0
LEVERAGE = 20
SL_CAPITAL_PCT = 0.10  # $1 max loss = 10% of $10
MAX_TRADES_DAY = 3

print("=" * 60)
print("  🧪 HQIP BACKTEST v5 — R:R = 1:2 to 1:4")
...
for i in range(60, len(df) - 8, 16):
    ...
    risk = capital * SL_CAPITAL_PCT  # $1 on $10
    sl_dist_pct = abs(sl_price - entry_price) / entry_price
    pos_value = min((risk / max(sl_dist_pct, 0.0001)) * LEVERAGE, capital * LEVERAGE)
    pnl = pct * pos_value / LEVERAGE
...
print(f"  ⚖️ نسبت Risk/Reward واقعی: 1:{rr_ratio:.1f}")
